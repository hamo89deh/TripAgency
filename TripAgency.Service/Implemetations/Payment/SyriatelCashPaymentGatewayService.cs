...
namespace TripAgency.Service.Implemetations.Payment
{
    public class SyriatelCashPaymentGatewayService : IPaymentGatewayService
    {
        ...
        private readonly string _syriatelCashWalletAddress;
        private readonly string baseUrl;

        public SyriatelCashPaymentGatewayService(IConfiguration configuration, ILogger<UsdtPaymentGatewayService> logger)
        {
            _logger = logger;
            _syriatelCashWalletAddress = configuration["PaymentGateways:SyriatelCash:WalletAddress"] ?? throw new InvalidOperationException("SyriatelCash WalletAddress not configured.");
        ...
        public async Task<Result<PaymentInitiationResponseDto>> InitiatePaymentAsync(PaymentRequest request)
        {
            ...
            var instructions = $"Please tranfer {request.Amount} {request.Currency} to a Wallet SyriatelCash : {_syriatelCashWalletAddress} . Save The Transfer Process (Transaction Hash) To send It later";
        ...
        public Task<Result<PaymentCallbackResult>> ProcessPaymentCallbackAsync(string transactionId, string statusFromGateway, Dictionary<string, string>? additionalParams = null)
        {
            ...
            _logger.LogWarning("SyriatelCash Manual Payment: Unexpected callback for {TransactionId} with status {Status}. This service does not process automatic callbacks.", transactionId, statusFromGateway);
